...
def gen_batch_function_orig(data_folder, image_shape):
    """
...
def gen_batch_function(data_folder, image_shape):
    ...
    :return:
    """
    def get_batches_fn(batch_size, shuffle=True):
        """
        Create batches of training data
        :param batch_size: Batch Size
        :return: Batches of training data
        ...
        for batch_i in range(0, len(image_paths), batch_size):
            images = []
            gt_images = []
            batch_idcs = indices[batch_i:batch_i+batch_size]
            for image_file in image_paths[batch_i:batch_i+batch_size]:
                gt_image_file = label_paths[os.path.basename(image_file)]
            ...
            yield np.array(images), np.array(gt_images), batch_idcs
    return get_batches_fn


def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, 
                    image_shape, is_training=None):
    """
...
def save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, 
                           keep_prob, input_image, is_training=None, 
                           saver=None):
    ...
    image_outputs = gen_test_output(sess, logits, keep_prob, input_image, 
        os.path.join(data_dir, 'data_road/testing'), image_shape, is_training)
